...
from .forms import CreateFormNotifications
...
class CreateViewNotifications( LoginRequiredMixin,View): 
    redirect_field_name = '/profile/login'
    #قالب کلی ایجاد 
    template_name = "create.html"
    extend = 'base.html'
    def get(self, request, *args, **kwargs):
        form = CreateFormNotifications()
    ...
    def post(self, request, *args, **kwargs):
        #فرم دریافت شده
        form = CreateFormNotifications(request.POST,request.FILES)
...
class ListViewNotifications(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        #دریافت تمام داده ها
        profileReciver = Profile.objects.get(user =self.request.user )
        queryset = Notifications.objects.all().filter( recivers = profileReciver  )
        list_data = []
        unread = 0
...
class ListViewAdminNotifications(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for query in queryset:
            ...
            data.append(query.lastName + '|' + str(query.employeeNumber))
            if(query.personalStatus =='notRead'):
                data.append('خوانده نشده')
            else:
                data.append('خوانده شده')
...
class UpdateViewNotifications( LoginRequiredMixin,View): 
    redirect_field_name = '/profile/login'
    template_name = "create.html"
    extend = 'base.html'
    def get_obj(self):
    ...
    def get(self, request,id=None ,*args, **kwargs):
        ...
        obj = self.get_obj()
        if obj is not None:
            form = CreateFormNotifications(instance=obj)
    ...
    def post(self, request,id=None ,*args, **kwargs):
        ...
        obj = self.get_obj()
        if obj is not None:
            form = CreateFormNotifications(request.POST or request.FILES,instance=obj)
            if form.is_valid():
                form.save()
...
def read_notifications(request , id):
    ...
    ntificationsSelected = Notifications.objects.get(id=id)
    ntificationsSelected.personalStatus = 'read'
    ntificationsSelected.save()
   
    return redirect('ListViewNotifications')

class DeleteViewNotifications( LoginRequiredMixin,View):
    redirect_field_name = '/profile/login'
    extend = 'base.html'
